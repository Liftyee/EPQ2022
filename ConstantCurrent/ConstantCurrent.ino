...
void setup() {
  ...
  */
  ...
  digitalWrite(ENABLE, LOW); // disable regulator
...
#define MAX_CUR_CONST 0.65
#define IMAX_PERIOD 10000
int lastStatus;
int lastIMax;
...
void loop() {
  ...
  if (millis() >= IMAX_PERIOD+lastIMax) {
    Serial.print("Recalibrating voltage...");
    // disable regulator
    digitalWrite(ENABLE, LOW);
    digitalWrite(RELAY, HIGH);
    // ina226.readAndClearFlags();
